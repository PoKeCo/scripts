...
(
    pids=()
    trap 'echo "${MAGENTA}SIGINT received, terminate child processes${NORM}"; kill "${pids[@]}" 2>/dev/null; exit 1' INT
    for cmp in $(ros2 topic find sensor_msgs/msg/CompressedImage);do
	decmp=${cmp}_decomp
	echo ${CYAN} Decomp ${cmp} to ${decmp}${NORM}
